...
export default function CalendarBooking({
  ...
  const config = {
    demo: {
      ...
      description: 'Choose an available time slot from my calendar. All times are shown in IST (Indian Standard Time) timezone.',
      consultationType: 'AI Demo',
      duration: '60-minute AI Demo session',
      apiEndpoint: '/api/demo-booking',
    },
    consultation: {
      title: 'Schedule Your Free Consultation',
      description: 'Book a 30-minute consultation to discuss your AI readiness assessment results.',
      consultationType: 'AI Readiness Consultation',
      duration: '30-minute consultation call',
      apiEndpoint: '/api/consultation-booking',
    }
  };

  const currentConfig = config[bookingType];
  ...
  const handleBooking = async () => {
    ...
    try {
      const bookingData = {
        ...
        consultationType: currentConfig.consultationType,
      };
      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          throw new Error(result.error || `Failed to book ${bookingType}`);
        }
      } else {
        throw new Error(`Failed to book ${bookingType}`);
      }
    } catch (error) {
      console.error('Booking failed:', error);
      alert(`Failed to book ${bookingType}. Please try again.`);
    } finally {
      setIsBooking(false);
    }
  };
  ...
  const renderTimeSlots = () => {
    ...
    return (
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
        {allSlots.map((slot) => renderSlot(slot))}
      </div>
    );
  };
    ...
    if (slot.type === 'available') {
      ...
      return (
        ...
        >
          <div className="font-medium">{slot.time}</div>
          <div className="text-xs opacity-75">
            {slot.isDemoBooking ? 'Booked' : 'Busy'}
          </div>
          {slot.subject !== 'Busy' && (
